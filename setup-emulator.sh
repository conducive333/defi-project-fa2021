...
flow transactions send ./cadence/transactions/fusd/setup.cdc $ADDR --signer local-account

# Install a OpenSpaceItems collection in the development account
printf "\n\n${BLUE}Setting up OpenSpaceItems collection...\n\n${NORMAL}"
flow transactions send ./cadence/transactions/open-space/setup.cdc $ADDR --signer local-account

# Fund the account with FUSD
printf "\n\n${BLUE}Sending FUSD to newly created account...\n\n${NORMAL}"
flow transactions send ./cadence/transactions/fusd/mint.cdc $ADDR 1000.0
